...
class FObject : public FRefCount {

public:

	class FAK_ASSUMED Search {

	public:

		Search(FClass* fclass) : index(-1), fclass(fclass) {}
		...
		inline void FindNext() {
			...
			do {
				...
				do {
					...
					if (index >= ms_apObjects.GetLength()) {
						ms_mMutex.Leave();
						return;
					}

					object = ms_apObjects[index];

				} while (!object);

				found = object->IsKindOf(fclass);
		...
		inline BOOL HasObject() const {
			...
			const BOOL found = index < ms_apObjects.GetLength();
		...
		inline FObject* GetObject() const {

			ms_mMutex.Enter();

			FAK_ASSERT(HasObject(), L"Object not found");

			FObject* object = ms_apObjects[index];
	...
	void operator delete(void* ptr, FObject* outer, const FName& object, const FName& storage, unsigned int flags);
...
using FObject::operator delete;						\
													\
void* operator new(size_t size, FObject* outer, const FName& object, const FName& storage, unsigned int flags = 0) {	\
	return StaticAllocateObject(StaticGetClass(), outer, object, storage, flags);	\
}													\
													\
static void StaticConstructor(void* data) {			\
...
static void StaticInitClass##class() {				\
	...
	self->Register();								\
													...
													\
static inline FClass* ms_pClass /* ; */
